$(document).ready(function() {
  ...
  function fasterPreview(uploader, element = null) {
    if (uploader.files && uploader.files[0]) {
      $(element || '#profileImage').attr('src', window.URL.createObjectURL(uploader.files[0]));
  ...
  $(document).on("submit", "form", async function (e) {
    if ($(this).data('ajax')) {
      const form_id = $(this).attr('id') ? '#'+$(this).attr('id')+' ' : '';
      ...
      try {
        ...
        if (response.data.success) {
          ...
          new swal({
            text: submit_button.data('validation') || response.data.message,
          })
        }
        hideLoader();
        ...
        if (error && error.response && error.response.status === 422) {
          if (submit_button.data('validation')) {
            ...
            Object.keys(errors).map(key => {
              ...
              if ($(form_id+'input[name="'+key+'"]') && !$(form_id+'input[name="'+key+'"]').hasClass('no-error-validation')) {
                $(form_id+'input[name="'+key+'"]').addClass('is-invalid');
                $('<div id="'+key+'" class="invalid-feedback">'+formatter.format(errors[orig_key])+'</div>').insertAfter($('input[name="'+key+'"]'));
              }
              if ($(form_id+'textarea[name="'+key+'"]') && !$(form_id+'textarea[name="'+key+'"]').hasClass('no-error-validation')) {
                $(form_id+'textarea[name="'+key+'"]').addClass('is-invalid');
                $('<div id="'+key+'" class="invalid-feedback">'+formatter.format(errors[orig_key])+'</div>').insertAfter($('textarea[name="'+key+'"]'));
              }
              if ($(form_id+'select[name="'+key+'"]') && !$(form_id+'select[name="'+key+'"]').hasClass('no-error-validation')) {
                $(form_id+'select[name="'+key+'"]').addClass('is-invalid');
  ...
  async function updateTable(elem) {
    const target = $('.search-data').data('target') || '#datas';
    showLoader();
    const response = await axios.get($(target).data('fetch') || window.location.href);
    hideLoader();
    $('.modal').modal('hide');
    $(target).html(response.data);
    $(elem).trigger("reset");
  ...
  if ($('#province').length) {
    function updateCities(province) {
      ...
      getProvinceCities(province).map(city => {
        $('#city').append($('<option>', {
          value: city,
          text: city,
          ...($('#city').data('value') === city ? { selected: true } : {}),
        }));
      });
    }
    getProvinces().map(province => {
      $('#province').append($('<option>', {
        value: province,
        text: province,
        ...($('#province').data('value') === province ? { selected: true } : {}),
      }));
    });
    if ($('#province').val()) {
  ...
  if ($('#city').length && ! $('#province').length) {
    ...
    $('#city').empty();
    $('#city').append("<option value='' selected='selected'>Select Location...</option>");
    getCities().map(city => {
      $('#city').append($('<option>', {
        value: city,
        text: city
  ...
  $(document).on('change', '.experience', function (event) {
    ...
    if ($(this).find(':selected').val() === 'With Experience') {
      ...
      $('input[name="years_of_experience"]').val('').prop('disabled', false);
    } else {
      form.find($('.years_of_experience')).addClass('d-none');
      $('input[name="years_of_experience"]').val('').prop('disabled', true);
    }
  });
  ...
  $(document).on('change', '.present', function (event) {
    const form = $(this).closest('form');
    if ($(this).is(':checked')) {
      form.find($('input[name="month_year_left"]')).val('').prop('disabled', true);
    } else {
      form.find($('input[name="month_year_left"]')).val('').prop('disabled', false);
    }
  });  
});